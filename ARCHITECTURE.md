...
- **AD-2, extending overwrite-occupancy checking to Tiers 0–2 — DECIDED (2026-08-20): do not extend. TASKS.md #46 closed.** If this were done, it would mean some Tier 0–2 requests that previously succeeded silently now stop and require explicit confirmation, the same flow Tier 3 already enforces — a **user-facing behavior change** (more friction on requests that used to just work), done in the name of safety. **The product decision: AD-1's "frontend guard is sufficient because it's the only write path" reasoning is accepted as adequate on its own.** The frontend's `overwriteGuard` is confirmed universal and unconditional across every write-shaped action type on every tier (§2.1/AD-1) — a backend-side check on Tiers 0–2 would be redundant defense-in-depth on top of an already-correct sole write boundary, not a closure of a real safety gap, and the added friction on ~85% of write traffic isn't justified by it. This decision is conditional on AD-1 continuing to hold (task-pane-only, `PRD.md` D1) — if a second write path is ever added, this reasoning must be revisited, not silently assumed to still apply.
...
3. ✅ **Resolved 2026-08-20 — §7.2's behavioral tradeoff.** Decision: do not extend occupancy checking to Tiers 0–2. See §7.2 for the recorded reasoning.
