...
Status: Phases 1–8 ALL IMPLEMENTED and unit-tested (Sept 23, 2026). Still **not** meeting §6's Definition of Done — the smoke harness has not yet passed end to end. See §0 for the honest scoreboard.
...
| 8 | **Resumability** | A dropped connection orphaning a long run with no way back | ✅ Done (Sept 23, 2026) — TASKS.md #293 (server side; client prompt still to wire) |
...
3. `Server/eval/ledger-smoke.ts` against the original booking-ledger prompt AND the 7 analogous prompts (Rental, Gym, Loan/EMI, Freelance Invoice, Attendance/Salary, Fleet/Fuel, Event/Wedding). Pass criteria: all sheets created, headers equal spec, no formula errors, no widths under 40pt.
   **Status: never actually done.** These 7 have been listed since the plan was written and have only ever been checked by running `ComputedColumnChecker` over them offline. Every live failure so far has come from the ONE prompt we do test. Until the other 7 run, "works for long prompts in general" is an assumption, not a finding.
   The harness also needs two checks it does not have: header text vs. the prompt's own column list, and formulas actually present in derived columns. It currently passes a sheet whose headers are wrong, which is how #283/#284 reached a live workbook.
...
| 8 | Sept 23, 2026 | #293 | `findResumableRun` + `resumableRun` on `getConversation`, so a build stranded by a dropped connection can be continued instead of rebuilt. Server side only — the client still needs to offer it. | +6 tests (phase8-resumability.spec.ts) | Server side done; client prompt outstanding |
| 3 | — | — | — | — | — |
| 4 | — | — | — | — | — |
| 5 | — | — | — | — | — |
